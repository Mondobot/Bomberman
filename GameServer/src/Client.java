...
public class Client extends Thread {
	...
	synchronized void putBomb()
	{
		...
		if (bombs.size()>1)
			return ;
		/*Map.Entry<Integer,Integer> bomb =
			    new AbstractMap.SimpleEntry<Integer, Integer>(pozX,pozY);
		bombs.add(bomb);*/
		
		Bomba bomba=new Bomba(server.getGameWorld(gameId),clientId,4,pozX,pozY);
		bombs.add(bomba);
		bomba.start();
	...
	void update() {
		sendMessage(server.getGameWorld(this.gameId).stringifyMap(this.clientId));
	}
}
